...
function createProvider({env=process.env,fetchImpl=(...args)=>fetch(...args)}={}){
 const configured=()=>!!env.OPENAI_API_KEY&&env.AI_SOLUTIONS_DISABLED!=='true';
 ...
 async function json(input,schema,name,verify=false){
  ...
  const response=await fetchImpl('https://api.openai.com/v1/responses',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+env.OPENAI_API_KEY},signal:AbortSignal.timeout(150000),body:JSON.stringify({model:model(),store:false,instructions:verify?instructions+'\nBạn đang kiểm tra một bản nháp. Tự tính lại và kiểm tra từng bước, đúng chương trình/phạm vi đã cung cấp, số liệu và hình. Không tin trạng thái ready của bản nháp. Chỉ đánh dấu đúng khi đã đủ căn cứ.':instructions,input:[{role:'user',content:input}],reasoning:{effort:'medium'},max_output_tokens:verify?3500:10000,text:{format:{type:'json_schema',name,strict:true,schema}}})});
  if(!response.ok){const code=response.status;throw Object.assign(Error(code===401||code===403?'Kết nối AI chưa được cấp quyền.':code===429?'AI đã chạm giới hạn sử dụng. Hãy thử lại sau.':'AI tạm thời chưa phản hồi được.'),{code:'AI_PROVIDER_'+code});}
