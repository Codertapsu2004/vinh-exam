...
function createService({q,pool,provider=createProvider(),env=process.env}){
 let busy=false,timer,initializing;
 async function initialize(){
  if(initializing)return initializing;
  initializing=(async()=>{const tx=await pool.connect();try{
   await tx.query('BEGIN');
   await tx.query('INSERT INTO ai_provider_health(id) VALUES(1) ON CONFLICT DO NOTHING');
   const state=(await tx.query('SELECT * FROM ai_provider_health WHERE id=1 FOR UPDATE')).rows[0];
   const hash=crypto.createHash('sha256').update((env.OPENAI_API_KEY||'').trim()+'|'+provider.model()).digest('hex');
   if(state.connection_hash!==hash){
    await tx.query('UPDATE ai_provider_health SET connection_hash=$1,issue=NULL,updated_at=now() WHERE id=1',[hash]);
    await tx.query("UPDATE ai_solution_items SET status='pending',reason='',error_code='' WHERE status='blocked' AND tries<3");
    await tx.query("UPDATE ai_solution_jobs SET status='queued' WHERE status='blocked'");
   }
   if(state.recovery_version<1){
    // One recovery attempt for the old release which discarded all provider error codes.
    const repaired=await tx.query("UPDATE ai_solution_items i SET status='pending',reason='',tries=LEAST(tries,2),updated_at=now() FROM ai_solution_jobs j JOIN assignments a ON a.ai_job_id=j.id WHERE i.job_id=j.id AND a.ai_enabled=true AND i.status='error' AND i.error_code='' RETURNING i.id");
    await tx.query("UPDATE ai_solution_jobs j SET status='queued' WHERE EXISTS(SELECT 1 FROM ai_solution_items i WHERE i.job_id=j.id AND i.status='pending')");
    await tx.query('UPDATE ai_provider_health SET recovery_version=1 WHERE id=1');
    if(repaired.rows.length)console.info('AI recovery: '+repaired.rows.length+' câu được xếp lại để xác định lỗi kết nối.');
   }
 ...
 async function health(){await initialize();return (await q('SELECT issue FROM ai_provider_health WHERE id=1')).rows[0]?.issue||null;}
 async function resetBlocked(){
  const rows=(await q("UPDATE ai_solution_items SET status=CASE WHEN tries<3 THEN 'pending' ELSE 'error' END,locked_at=NULL,updated_at=now() WHERE status='blocked' RETURNING job_id")).rows;
  for(const id of new Set(rows.map(x=>x.job_id)))await refresh(id);
 }
 async function retry(jobId){
  await initialize();const tx=await pool.connect();try{
   await tx.query('BEGIN');const issue=(await tx.query('SELECT issue FROM ai_provider_health WHERE id=1 FOR UPDATE')).rows[0]?.issue;
   if(issue&&Date.now()<Date.parse(issue.retryAt))throw Object.assign(Error('Kết nối AI đang tạm dừng. Hãy chờ ít nhất đến '+new Date(issue.retryAt).toLocaleTimeString('vi-VN',{timeZone:'Asia/Ho_Chi_Minh'})+' rồi thử lại.'),{status:429});
   const rows=(await tx.query("UPDATE ai_solution_items SET status='pending',reason='',error_code='',solution=NULL,updated_at=now() WHERE job_id=$1 AND status IN('error','needs_review','blocked') AND tries<3 RETURNING id",[jobId])).rows;
   // Grant exactly one recovery probe, even for a new job sharing a blocked connection.
   if(issue)await tx.query('UPDATE ai_provider_health SET issue=$1,updated_at=now() WHERE id=1',[JSON.stringify({...issue,automatic:true,retryAt:new Date().toISOString()})]);
   await tx.query('COMMIT');await refresh(jobId);return rows.length;
  }catch(error){await tx.query('ROLLBACK').catch(()=>{});throw error;}finally{tx.release();}
 }
 ...
 function policy(body,a){
  ...
  if(p.ai&&env.AI_SOLUTIONS_DISABLED==='true')throw Object.assign(Error('Đang dùng chế độ không gọi API. Hãy nhập lời giải có sẵn để công bố.'),{status:400});
 ...
 async function summary(jobId,includeSolutions=false){
  ...
  const items=(await q(`SELECT question_id,position,status,reason,error_code,tries${includeSolutions?',solution,question':''} FROM ai_solution_items WHERE job_id=$1 ORDER BY position`,[jobId])).rows;
  const issue=await health(),remaining=items.filter(x=>['pending','running','blocked'].includes(x.status)).length;
  return {...job,status:issue&&remaining?'blocked':job.status,issue:remaining?issue:null,remaining,done:items.filter(x=>['ready','needs_review','error'].includes(x.status)).length,total:items.length,ready:items.filter(x=>x.status==='ready').length,errors:items.filter(x=>x.status==='error').length,needsReview:items.filter(x=>x.status==='needs_review').length,items};
 }
 async function refresh(jobId){
  ...
  const status=tally.blocked?'blocked':tally.pending||tally.running?'running':tally.needs_review||tally.error?(tally.ready||tally.needs_review?'partial':'failed'):'ready';
  await q('UPDATE ai_solution_jobs SET status=$1,updated_at=now() WHERE id=$2',[status,jobId]);
 }
 async function runOne(){
  if(!provider.configured())return false;
  const issue=await health();
  if(issue){
   if(!issue.automatic||Date.now()<Date.parse(issue.retryAt))return false;
   await q('UPDATE ai_provider_health SET issue=NULL,updated_at=now() WHERE id=1');await resetBlocked();
  }
  ...
  try{
   ...
   const message=error.name==='TimeoutError'?'AI phản hồi quá lâu. Có thể thử lại.':error.message?.startsWith('AI ')||error.code?.startsWith('AI_')||error.needsReview?error.message:'Lời giải hoặc hình AI trả về chưa đúng cấu trúc. Cần kiểm tra câu hỏi hoặc thử lại.';
   const code=/^AI_[A-Z_]+$/.test(error.code||'')?error.code:'AI_OUTPUT';
   await q('UPDATE ai_solution_items SET status=$1,reason=$2,error_code=$3,locked_at=NULL,tries=GREATEST(0,tries-CASE WHEN $5 THEN 1 ELSE 0 END),updated_at=now() WHERE id=$4',[error.pause?'blocked':error.needsReview?'needs_review':'error',String(message).slice(0,2000),code,item.id,!!error.pause]);
   console.error('AI provider result: '+JSON.stringify({code,httpStatus:error.httpStatus||null,providerCode:error.providerCode||null}));
   if(error.pause){const issue={code,message,automatic:!!error.automatic,retryAt:new Date(Date.now()+(error.retryAfterMs||60000)).toISOString()};await q('UPDATE ai_provider_health SET issue=$1,updated_at=now() WHERE id=1',[JSON.stringify(issue)]);}
  }
 ...
 return {provider,defaults,policy,enqueue,applyPolicy,summary,health,retry,runOne,pump,start,stop,enrich};
}
